    ...
    <el-form :model="formData" method="post" action="/schadmin/import/importScore" enctype="multipart/form-data" id="importScoreForm">
      <ol>
        <li style="height:80px">
          ...
          <p style="float: left;margin-top: 10px;">点击下载普通考试模板&nbsp&nbsp&nbsp&nbsp&nbsp<a href="http://60.205.115.173:8888/yitai-educational/downloads/普通考试成绩模板.xls" target="_blank"><el-button type="primary" class="icon-download download">   下载</el-button></a></p>
          <p style="float: left;margin-top: 10px;">点击下载学业水平考试模板&nbsp&nbsp&nbsp&nbsp&nbsp<a href="http://60.205.115.173:8888/yitai-educational/downloads/学业水平考试模板.xls" target="_blank"><el-button type="primary" class="icon-download download">   下载</el-button></a></p>
        </li>
        <li>
          <div style="padding-left:13px;height:34px;background:#f9f9f9;width: 100%;">
            <h3>2、选择导入试卷</h3><!-- <el-button class="" icon="  "  v-on:click="addNewPaper">新增试卷</el-button> <el-button class="editbtn" v-on:click="editPapers">编辑试卷</el-button>-->
          ...
          <el-form-item label="试卷名称：" style="margin-top: 20px">
            <el-select v-model="deafultPaperId" class="inputWidth" placeholder="请选择试卷">
        ...
        <li class="file-box">
          ...
          <ul class="el-upload-list el-upload-list--text">
            <el-row>
              <el-col style="padding-right:100px;" :span="12" v-for="(value, index) in excelFile" :key="excelFile"><li style="padding:0;" class="el-upload-list__item is-success">
    ...
    <el-dialog class="tankuang" :title="dialogTitle[addOrEdit] + '试卷'" v-model="dialogFormVisible">
      <el-row>
        ...
        <el-col :span="addOrEdit === 1 && 6 || 0" v-if="addOrEdit === 1" v-model="paperList">
          <ul>
            <li v-for="(paper, index) in paperList" :key="index"><el-button style="min-width: 200px;" v-on:click="editPapers(paper.id)" :type="(paper.id == dialogForm.id) ? 'primary' : 'text'">{{paper.name}}</el-button></li>
          </ul>
        </el-col>
        <el-button v-if="dialogForm.id" style="float: right" v-on:click="deletePaper(dialogForm.id)">删除</el-button>
        <el-col :span="addOrEdit === 1 && 18 || 14">
          <el-form>
            <el-form-item label="试卷名称：" :label-width="formLabelWidth">
              <el-input auto-complete="off"   placeholder="请填写试卷名称，同一试卷请勿重复新增" v-model="dialogForm.name"  style="width:70%;" :maxlength="30"></el-input>
            ...
            <el-form-item label="学年：" :label-width="formLabelWidth">
              <!--  <el-select v-model="dialogForm.schoolYear" style="width:70%;">
            ...
            <el-form-item label="学期：" v-model="dialogForm.term" :label-width="formLabelWidth" >
              ...
              <p v-if="dialogForm.term == 0">上学期</p>
              <p v-if="dialogForm.term == 1">下学期</p>
            </el-form-item>
            <el-form-item label="考试类型：" :label-width="formLabelWidth" >
              <el-radio-group v-model="dialogForm.type" v-if="addOrEdit ===  0">
                <el-radio :label="1">期中考试</el-radio>
                <el-radio :label="2">期末考试</el-radio>
                <el-radio :label="3">必修考试</el-radio>
                <el-radio :label="4">选修考试</el-radio>
                <el-radio :label="5">月考</el-radio>
                <el-radio :label="6">普通模拟考试</el-radio>
                <el-radio :label="7">学业水平考试</el-radio>
                <el-radio :label="8">其他</el-radio>
              </el-radio-group>
              <p v-if="dialogForm.type =='1'&&addOrEdit ===1">期中考试</p>
              <p v-if="dialogForm.type =='2'&&addOrEdit ===1">期末考试</p>
              <p v-if="dialogForm.type =='3'&&addOrEdit ===1">必修考试</p>
              <p v-if="dialogForm.type =='4'&&addOrEdit ===1">选修考试</p>
              <p v-if="dialogForm.type =='5'&&addOrEdit ===1">月考</p>
              <p v-if="dialogForm.type =='6'&&addOrEdit ===1">普通模拟考试</p>
              <p v-if="dialogForm.type =='7'&&addOrEdit ===1">学业水平考试</p>
              <p v-if="dialogForm.type =='8'&&addOrEdit ===1">其他</p>
            </el-form-item>
            <el-form-item class="dialog-footer" style="margin-left:25%;">
...
<script>
  ...
  export default {
    ...
    mounted() {
      ...
      that.$ajax('/teacher/findExaminationByCondition', {schoolId:that.dialogForm.schoolId})
        .then(res => {
          if(res.code == 200 && res.data && res.data.length) {
            ...
            //this.deafultPapername=this.papers[0].name;
            this.deafultPaperId=this.papers[0].id;
            this.deafultTerm=this.papers[0].term;
            this.deafultSchoolYear=this.papers[0].schoolYear;
          }
        });
      //获取学年列表
    ...
    methods: {
              ...
              .then(res => {
                if(res.code == 200 && res.data) {
                  ...
                  if(that.dialogForm.id) {
                    ...
                    //that.deafultPaperId = that.dialogForm.id.toString();
                  }
      ...
      deletePaper: function(paperId) {
        ...
        that.$ajax('/schadmin/deleteExaminationById', {id: paperId})
          .then(res => {
            if(res.code == 200) {
      ...
      submitForm: function(e) {
        ...
        let schoolYear= this.formData.schoolYear || this.deafultSchoolYear;
        let term= this.formData.term || this.deafultTerm;
        ...
        if(examId=="" ){
          this.$message({message: '请选择导入的试卷',type: 'warning'});
          return;
        }
        let that = this
        ...
        fd.append('term',term);
        $.ajax({
          url:"http://60.205.115.173:8888/yitai-educational/schadmin/import/importScore?",
          type:"post",
          data:fd,
          dataType: 'json',
          processData:false,
...
<style >
  #Subjectbatch .saveDiv {
    padding: 10px 0 10px 0;
    height: 30px;
    line-height: 30px;
    border-bottom: 1px solid #e9edf1;
  }
  #Subjectbatch .saveDiv a {
    font-size: 14px;
    color: #666;
    line-height: 30px;
  }
  ...
  #Subjectbatch .editbtn{
    background: #f9f9f9;
  }
  #Subjectbatch .el-form-item{
    margin-left:30px;
  }

  #Subjectbatch .file-box{ position:relative;width:100%;margin: 0px;}
  #Subjectbatch .txt{ height:28px; border:1px solid #e6e6e6; width:180px;margin: 0px;}
  #Subjectbatch .btn{ background-color:#FFF; border:1px solid #e6e6e6;height:30px; width:70px;margin: 0px;}
  #Subjectbatch .upload-demo{ position:absolute; top:0; left:0px; height:32px; ;width:260px ;}
  ...
  /*新增-编辑弹框*/
  #Subjectbatch .el-dialog--small{
    min-width: 942px;
  }
  #Subjectbatch  .el-dialog__header {
    text-align: center;
    padding: 0;
    line-height: 32px;
  ...
  #Subjectbatch .el-dialog__title{
    font-size: 12px;
    color:#0d9ee9;
  }
  #Subjectbatch  .el-dialog__headerbtn{
    margin-right: 20px;
  }
  /*下*/
  #Subjectbatch .el-dialog__body {
    padding: 30px 30px;
    color: #48576a;
    font-size: 14px;
  }
  #Subjectbatch .el-dialog__body .el-row{
    margin-left: 0;
  }
  #Subjectbatch .el-dialog__body .el-row  .el-form-item{
    margin-left: 0;
  }
  #Subjectbatch  .dialog-footer{
    margin-left: 300px;
  }
  #Subjectbatch  .el-radio-group{
    width: 410px;
    padding-top: 10px;
  }
  #Subjectbatch  .el-radio{
    margin-left: 0px;
    line-height:16px;
    margin-bottom: 10px;
    margin-right: 15px;
  }
  #Subjectbatch .addbtn,.editbtn{
    border: none;
    list-style: none;
  }
  /*上传部分*/
