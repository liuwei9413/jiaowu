<template>
  <div id="schoolBaseInfo" style="text-align: left">
    <div class="main-content" v-loading.fullscreen.lock="fullscreenLoading">
      ...
      <div class="title"><i></i>
        <span class="bluespan">基础信息</span>
        <div style="float: right;">
          <el-button style="float:right;" class="white-color add-btn el-button  icon-conserve el-button--primary" type="primary" v-if="isShowSchool" @click="changeShowSchool" > 编辑</el-button>

          <el-button  class="white-color el-button  icon-conserve  el-button--primary" type="primary" v-if="!isShowSchool" @click="cancelSaveClick" > 取消</el-button>
          <el-button class="white-color add-btn el-button  icon-conserve el-button--primary" type="primary" v-if="!isShowSchool"  @click="submitForm('ruleForm')"  > 保存</el-button>
        </div>
      </div>
      ...
      <el-form-item label="学校名称：" prop="name" :class="{'no-star': isShowSchool!=false}">
        <el-input v-model.trim="ruleForm.name" v-show="isShowSchool == false" :maxlength="30" style="width:288px;"></el-input>
        <p v-show="isShowSchool != false" >{{ruleForm.name}}</p>
      </el-form-item>
      <el-form-item label="学校类型：" prop="schoolType" :class="{'no-star': isShowSchool!=false}">
        <el-select name="schoolType" id="ruleForm.schoolType" v-model="ruleForm.schoolType" style="width:288px;" v-on:change="changeSchoolType" v-show="isShowSchool == false">
      ...
      <el-form-item label="学制：" prop="codeType" :class="{'no-star': isShowSchool!=false}">
        ...
        <el-select  @change="changeSchooling" v-bind:disabled="ruleForm.schoolType === '' || ruleForm.schoolType > 8 || ruleForm.schoolType == 2" v-show="isShowSchool == false" :placeholder="codeTypeCovert.showStr" v-model="codeTypeCovert.saveVal">
      ...
      <el-form-item label="教学范围：" prop="region" class="region">
        ...
        <p v-show="isShowSchool != false" >{{convertGrads(grads)}}</p>
      </el-form-item>
      <el-form-item label="地理位置：" prop="position"  :class="{'no-star': isShowSchool!=false}">
        <el-select v-model="ruleForm.position" style="width:288px;" v-show="isShowSchool == false">
      ...
      <el-form-item label="教师数量：">
        <p>{{ruleForm.teacherCount}}</p>
      </el-form-item>
      <el-form-item label="学生数量：">
        <p>{{ruleForm.studentCount}}</p>
      </el-form-item>
      <!--------------------------学校地址-------------------------------->
      <el-form-item label="学校地址：" prop="address">
        ...
        <p v-show="isShowSchool != false" >{{( selectedCounty.name || '')+( selectedCity.name || '')+( selectedProvince.name || '')+(ruleForm.address)}}</p>
      </el-form-item>
      <!--------------------------学校地址结束-------------------------------->
      ...
      <div class="tabs-body">
        <el-dialog title="信息提示"  :close-on-click-modal="false" :show-close="false" v-model="showSchoolYearDialog">
...
//        		codeMaxVal = eachLists[j].codeIndex;
      ...
      getSchoolDetail(){
        ...
        this.$ajax(schoolDetailPath)
          .then(res => {
              ...
              this.codeVal =( res.data.codeType == '5-4' ? 2 : (res.data.codeType == '6-3' ? 1 : '') );
              this.fullscreenLoading = false;
              this.oldSchoolInfo = {
                codeVal: ( res.data.codeType == '5-4' ? 2 : (res.data.codeType == '6-3' ? 1 : '') ),
                data: {
                  name: res.data.name || '',
      ...
      submitForm(formName) {
        var that = this;
        this.ruleForm.codeType = this.codeTypeCovert.saveVal == 1 ? '6-3' : '5-4';
        //选中教学范围
        //修改验证及请求
...
//                  this.buildGrade(); //调用新增年级接口（后台自动生成年级）
                  this.$alert('修改成功', '提示', {
                    ...
                    callback: action => {
                    	this.isShowSchool = true;
//                      this.$router.replace('/schoolManagement');
                    }
                  });
                }else{
