<template>
 <div class="scoreManagement publicScore">
   <div class="tabBtn textLeft btns">
     <el-button type="primary"  @click="goSubjectBatch"><i class="icon-leading-in svgStyle"></i>批量导入</el-button>
...
//    beforeMount: function () {
    mounted: function () {
      ...
      this.$ajax('schadmin/findScoreClassByCondition',
        {"schoolId":schoolId}
      )
...
//      下载
      handleUpload(index, row) {
        ...
        let $this = this;
        let termState = row.term == '上学期' ?  0 : 1;
        location.href = this.staticResources+'schadmin/downloadClassStudentScore?'+"classId="+row.classId+"&examId="+$this.value+"&term="+termState+"&schoolYear="+$this.yearTime;
//        this.$ajax('schadmin/downloadClassStudentScore',
//          {"classId" : row.classId,"examId":$this.value ,"term":row.term,"schoolYear":$this.yearTime}
//        ).then(function (response) {
//          if (response.code == 200){
//            alert("下载成功！")
//          }else {
...
//        location.href = '/querySubjectScore?data=1';
        this.$router.push({ path: '/scoreManagement/querySubjectScore',  query: {"examId":this.value,"classId":row.classId}});
...
//        this.tableData3.splice(index, 1);
        this.$confirm('此操作将永久删除该文件, 是否继续?', '提示', {
          ...
          this.$ajax('schadmin/deleteScoreByClassIdAndExamId',
      ...
      batchLoad(){
        ...
        if (this.multipleSelection && this.multipleSelection.length >0){
          ...
          this.$confirm('您将批量下载选中的文件, 是否继续?', '提示', {
            ...
            location.href = this.staticResources+'schadmin/downloadClassStudentScore?'+"classId="+downLoadId+"&examId="+$this.value+"&schoolYear="+$this.yearTime+"&term="+$this.term;
//            this.$ajax('schadmin/downloadClassStudentScore',
//            {"classId":downLoadId, "examId":$this.value,"schoolYear":$this.yearTime}
//          ).then(function (response) {
//            if (response.code == 200){
//              alert("下载成功！")
//            }else {
...
//      跳转批量导入
      goSubjectBatch(){
        this.$router.push({ path: '/scoreManagement/subjectbatch',  query:{'schoolId':this.userInfo.schools.schoolId}});
      }
    },
  }
