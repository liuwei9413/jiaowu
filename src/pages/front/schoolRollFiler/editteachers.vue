    ...
    <div id="editteachers">
      ...
      <el-form ref="form" :model="teacherInfoForm" :rules="teacherInfoRules" :show-message="type != 'detail'" label-width="200px" class="edit"
        ...
        <el-form-item class="note" label="近三年获奖或发表文章情况：（按照奖项、获奖时间、获奖级别、获奖内容顺序填写）" prop="note">
          <el-input type="textarea" v-model.trim="teacherInfoForm.note" class="item-text"></el-input>
  ...
  export default {
    ...
    methods: {
      ...
      GetVercode(){
        ...
        this.$ajax('/schadmin/changeLoginName',{oldPhone:_self.oldtel,newPhone:_self.newtel,isSchoolHeader:0}).then(res => {
            		if(res.code == 200) {
                  this.$message({
                    title: '成功',
                    message: '修改成功',
