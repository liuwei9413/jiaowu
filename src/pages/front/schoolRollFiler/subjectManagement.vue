  ...
  <div class="subjectManagement" v-loading.lock="fullscreenLoading">
    ...
    <div class="subjectMgt-body">
      <div class="subjectMgt-body-tool" v-if="teachingInfo != 0 ">
        <el-button type="success" @click="addNewSubject(currentGradeId)"  icon="plus" v-if="subjectState">新增科目</el-button>
      </div>
      ...
      <div class="noData" v-if="teachingInfo == 0" >
        <p class="empty-text">暂无科目信息</p>
      </div>
      
      <div class="table-wrap" :class="[{ scroll: subjectInfo.length > 8}]">
        <table class="table-striped" v-if="teachingInfo != 0">
          <thead>
            <tr>
              <th><div class="cell">班级</div></th>
            ...
            <tr v-for="(item,index) in teachingInfo" :key="index">
              ...
              <td v-for="(list,index) in item.subjectInfo"  >
                ...
                <div class="tooltip-warp" :class="[{ fullCell: !list.teachingInfo.teacherName }]">
                  ...
                  <a href="javascript:;" class="teacherName"  :class="[{ fullCell: !list.teachingInfo.teacherName },{ 'icon-new': !list.teachingInfo.teacherName }]" @click="getTarget(item,list)" >{{ list.teachingInfo.teacherName }}</a>
                  ...
                  <div class="tooltip-content"  v-if=" list.teachingInfo.teacherName != '' ">
                    <div class="item" @click="getTarget(item,list)">
    ...
    <el-dialog title="添加科目教师信息" v-model="dialogVisibleEditTeacher"  :close-on-click-modal="false" size="tiny" >
      ...
      <!-- :filter-method="" -->
        ...
        >
          <el-select v-model="teacherEdit.currentTeacherId"   :placeholder="defaultTeacher" no-data-text="请前往教师管理界面维护教师信息">
            <el-option
    ...
    <el-dialog title="新增科目" v-model="dialogVisibleAddSubject"  :close-on-click-modal="false" size="tiny" >
...
<script>
  export default {
    ...
    watch:{
      'teacherEdit.currentTeacherId':function(newVal,oldVal){
        // console.log(newVal)
        if(newVal == 0){
    ...
    methods:{
      ...
      getTarget(item,list){
        ...
        // this.defaultTeacher = list.teachingInfo.teacherName;
        // this.defaultTeacher = list.teachingInfo.teacherName;
      ...
      teacherSaveData(){
        ...
        /*if(this.defaultTeacherId != ''){
          this.teacherSave(this.defaultTeacherId);
        }*/

        this.$refs.teacherEdit.validate((valid) => {
         if (valid) {
      ...
      deleteSubject(item){
        ...
        if( haveTeacher() ){
          ...
          this.$alert('该学科下存在教师，请先清空教师学科之间的关系', '提示', {
