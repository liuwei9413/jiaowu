  ...
  <div id="schoolRollteacherbatch">
    ...
    <el-form method="post" action="/schadmin/import/importScore" enctype="multipart/form-data" id="importScoreForm">
      <ol>
        <li class="xiazai">
          <h3>1、下载模板</h3>
          <p >点击下载导入模板&nbsp&nbsp&nbsp<a href="http://60.205.115.173:8888/yitai-educational/downloads/参测教师基本信息表.xls" target="_blank"><el-button type="primary" class="icon-download">   点击下载</el-button></a></p>
        </li>
        <li class="file-box">
  ...
  export default {
    ...
    methods: {
      ...
      submitForm: function(e) {
        ...
        $.ajax({
          url:'http://60.205.115.173:8888/yitai-educational/schoolProvider/importTeacher?type=teacher&schoolId='+that.userInfo.schools.schoolId,
          type:"post",
          data:formData,
          dataType: 'json',
          processData:false,
...
<style>
  #schoolRollteacherbatch{
    min-height:800px;
    padding: 0 20px  50px 20px;
  }
  #schoolRollteacherbatch .text-right1{
   padding:10px 0 10px 0;
    height: 30px;
    line-height: 30px;
    border-bottom: 1px solid #e9edf1;
  }
  #schoolRollteacherbatch .text-right1 a {
    font-size: 14px;
    color: #666;
    line-height: 30px;
  }
  ...
  #schoolRollteacherbatch li{
    padding-top: 20px;

  }

  #schoolRollteacherbatch .xiazai p{
    font-size:14px;
    color:#666666;
    margin-top:15px;
    margin-left: 36px;
  }
  #schoolRollteacherbatch h3{
    background:#f9f9f9;
    height: 34px;
    line-height: 34px;
    padding-left: 13px;
  ...
  #schoolRollteacherbatch .el-form-item{
    margin-top: 15px;
    margin-left:34px;
    margin-bottom: 0;
  }
  ...
  #schoolRollteacherbatch  .explain h2{
    font-size: 14px;
    color:#666;
    line-height:34px;
  }
  #schoolRollteacherbatch  .explain  p{
    text-indent:2em;
    font-size: 14px;
    color:#666;
    line-height:23px;
  ...
  /*导入*/
  #schoolRollteacherbatch  .file-box{ position:relative;width:100%;margin: 0px;}
  #schoolRollteacherbatch  .txt{ height:28px; border:1px solid #e6e6e6; width:180px;margin: 0px; position: relative;left:5px;top:-1px;}
  #schoolRollteacherbatch  .btn{ background-color:#FFF; border:1px solid #e6e6e6;height:30px; color:#0d9ee9;margin: 0px;}
  #schoolRollteacherbatch .upload-demo{ position:absolute; top:1px; left:0px; height:32px; opacity: 0;width:260px ;}

  /*导入*/
  #schoolRollteacherbatch  .submit{
    position: relative;
    top: 1px;
    height:30px;
    width:80px;
  ...
  #schoolRollteacherbatch  .submit:hover{
    position: relative;
    top: 1px;
    height:30px;
    width:80px;
